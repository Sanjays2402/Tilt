...
final class DepthRenderer {
    ...
    init?() {
        ...
        do {
            // The shaders live as a real Metal file in the app's resources,
            // compiled at launch so a shader edit never touches Swift.
            guard let shaderURL = Bundle.module.url(forResource: "Shaders", withExtension: "metal") else {
                Log.geometry.error("metal shaders missing from the app bundle")
                return nil
            }
            ...
            Log.geometry.error("metal pipeline failed: \(String(describing: error), privacy: .public)")
            return nil
        }
    ...
    nonisolated func makePicture(image: CGImage, screenSize: CGSize, pixelScale: CGFloat) -> PreparedPicture? {
        ...
        case .failed:
            Log.geometry.error("could not build the mip chain for the picture")
            return nil
        }
        commands.commit()
    ...
    func beginLive(screenSize: CGSize, pixelScale: CGFloat) -> Bool {
        ...
        layer.colorspace = CGColorSpace(name: CaptureEngine.colourSpaceName)
    ...
    func seed(image: CGImage) -> Bool {
        ...
        guard let staging = device.makeBuffer(length: bytesPerRow * height, options: .storageModeShared),
              ...
              let space = CGColorSpace(name: CaptureEngine.colourSpaceName),
